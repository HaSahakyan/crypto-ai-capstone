...
class TargetCreatorSimple:
    def __init__(self, df: pd.DataFrame, price_type='Close', pct_threshold=0.01, min_length=3, smoothing_factor=0.0):
        """
    ...
    def add_trend_columns(self, direction=['Upward', 'Downward']):
        """
        ...
        if not hasattr(self, 'up_trends') or not hasattr(self, 'down_trends'):
            self.detect_trends(flactuation_length=5, flactuation_threshold=0.01)
    ...
    def visualize_trends(self):
        ...
        ax.set_xlabel('Date')
        ax.set_ylabel('Price')
        ax.set_title(f'Trend Segments in {self.price_type.capitalize()} Price')
